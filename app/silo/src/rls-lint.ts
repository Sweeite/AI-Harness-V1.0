...
export type RlsRule = "initplan-unwrapped" | "table-uncovered";
...
const GUARDED_CALL = /\b(user_perms|user_clearances|user_restricted|user_aal|auth\.uid|auth\.jwt|auth\.role)\s*\(/i;
...
export function checkAllRls(files: { tag: string; sql: string }[]): RlsFinding[] {
  ...
  return [...initplan, ...checkCoverage(files)];
}
