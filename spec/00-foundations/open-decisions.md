...
- **Status:** 🟡 OPEN — a coupling fork for C5. Everything ships fail-safe (#1/#2/#3): no task is abandoned, the paused set is durable, resume re-checks authz. Decide (A/B/C) when the Checkpoint-5 integration wires the DSC pause into the live dispatcher.
...
- **Status:** 🟡 OPEN — a small change-controlled edit owed to ISSUE-061 at the Checkpoint-5 reconcile. Does not block ISSUE-064 (its own paths are canonical + write-boundary-guarded).
