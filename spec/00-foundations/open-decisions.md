...
## OD-195 — Boot health probe: treat a 4xx (invalid service_role key) as unhealthy, or reachability-only? ⚪ OPEN (recommendation)
...
- **Status:** ⚪ OPEN — operator: confirm (A) (tighten + test) or (B) (defer to live smoke). Not a Stage-5 blocker; it is a boot-gate hardening. See `logic-bug-sweep-findings.2026-07-07.md` (UNCERTAIN).

## OD-196 — write-gate `executeApproved` defense-in-depth: re-read the proposal vs trust the decision object? ⚪ OPEN (low urgency)
...
- **Status:** ⚪ OPEN (low urgency) — revisit at ISSUE-056. Add the seam-contract comment now regardless. Not a Stage-5 blocker. See `logic-bug-sweep-findings.2026-07-07.md` (UNCERTAIN).
