...
## OD-191 — Approval-queue view: tier/floored/routing/soft-countdown are not persisted (the live queue can't be rebuilt from the DB) 🟢 RESOLVED (defer C6 surface)
...
- **Status:** 🟢 RESOLVED (2026-07-07, operator delegated to recommendation) — **defer the C6 operator-queue surface + its decoration-persistence delta** until that surface is actually built (don't build persistence for an unbuilt UI). **Immediate sub-fix (owed, small):** make `buildQueueView` throw a loud "decoration persistence owed (OD-191)" error instead of returning the silently-empty ref (#3). Nothing in Stage 5 depends on the live queue view. See `live-adapter-backfill-findings.2026-07-07.md` (B3).

## OD-192 — Invite lifecycle (revoke / reissue / resend / mark-bounced) has no persistent table 🟢 RESOLVED (model on profiles)
...
- **Status:** 🟢 RESOLVED + ✅ IMPLEMENTED (2026-07-07 session 74) — lifecycle IS v1 (specced FR-0.INV.003/.005/.007) → **modelled on the pending-`profiles` row** (Option A, no new table). **Migration `0027_profiles_invite_lifecycle`** adds `profiles.revoked_at` + `bounced_at` (additive nullable timestamptz), **APPLIED LIVE** (silo head 0026→0027). Adapter: `revokeInvite` stamps `revoked_at` (txn + `for update`; USED = no-op; idempotent); `markBounced` stamps `bounced_at` (delivery axis; idempotent; +event+audit); `reissue`/`resend` re-deliver a still-pending invite gated by `loadInviteLive`; **`loadInviteLive` now rejects a revoked token (#2)** and reflects `bounced_at` as delivery=bounced (#3). Live-verified: `invite-seed/results/live-smoke.sql` #I1/#I2/#I3 ALL PASS (rolled back, R10); offline 34/34; silo 65/65. **Residual (tracked):** reissue's true server-side native-token refresh (Supabase `generateLink`) needs an unbuilt `AuthAdmin` seam — **AF-074**; expiry/consumption stay server-side (OD-014), so no expiry column owed. Fail-loud stopgap (5fcbbdf) superseded by the full impl (3a3d193). See `live-adapter-backfill-findings.2026-07-07.md` (B5).

---

## OD-193 — Runtime DB connection role for silo adapters: `postgres` owner vs `service_role` (undecided; code + canary disagree) 🟢 RESOLVED (ratify postgres owner)
...
- **Status:** 🟢 RESOLVED (2026-07-07, operator delegated to recommendation) — **ratify `postgres`/owner as the adapter runtime role** (Option A): it is what provisioning wires and what works today (retention DELETEs succeed); RLS stays defence-in-depth for a future authenticated/PostgREST path. **Owed (doc-only, no migration/reconnect):** correct ADR-006 + the misleading "service_role" comments across the adapters to say owner/RLS-bypass, so a future RLS-grant audit isn't misled. The config-store retention-DELETE "MAJOR" is thereby a non-issue under the ratified role. See `live-adapter-backfill-findings.2026-07-07.md` (Wave A).

---

## OD-194 — approval-tiers adapter is non-functional live: string action-name / reviewer-identity bound into uuid/FK columns 🟢 RESOLVED-DIRECTION (wire id resolution)
...
- **Status:** 🟢 RESOLVED-DIRECTION (2026-07-07, operator delegated to recommendation) — **(A) wire the id resolution**: thread the real `task_queue.id` + reviewer `profiles.id` (uuids) into `tierAndGate`/`raiseFlag`/`resolve` instead of the string names, finishing the half-wired adapter. **Immediate sub-fix (owed):** B3 `buildQueueView` fail-loud (per OD-191). **Implementation owed (fresh chat):** needs the caller contract pinned — where the `task_queue.id` + reviewer `profiles.id` come from at gate time (the C6 wiring). Until wired, approval-tiers stays non-functional live (fail-loud `22P02`, not silent). See `live-adapter-backfill-findings.2026-07-07.md` (approval-tiers).

---

## OD-195 — Boot health probe: treat a 4xx (invalid service_role key) as unhealthy, or reachability-only? 🟢 RESOLVED + IMPLEMENTED (Option A)

- **Surfaced by:** logic-bug sweep (session 74), `app/service/src/health.ts:53` (`probeSupabase`), verdict UNCERTAIN (design, not a code-vs-intent contradiction). `probeSupabase` returns `res.status < 500`, so a PostgREST **401/403** (a present-but-INVALID `SUPABASE_SERVICE_ROLE_KEY` — rotated / typo / wrong project) is classified **reachable → healthy**; `checkHealth` returns `{ok:true}` → HTTP 200 → **Railway routes production traffic to a silo whose service-role key cannot read/write the client DB.** The deploy looks green while the DB layer is unusable (#2 routing to a non-functional silo · #3 silent half-config).
- **Why it's a decision, not a bug:** line 52's comment deliberately documents *reachability-only* semantics ("any HTTP response proves reachability"), and the code faithfully implements that. An offline boot probe fundamentally cannot prove a key actually reads/writes without an authed round-trip — the repo's own R10 / live-adapter-hygiene protocol defers real-DB key validation to a separate **live smoke**. So widening the gate to reject a 401 changes the gate's *documented* semantics → change-control. BUT the file's own header (L6-8) aspires to "200 ONLY when every required secret is present AND the client Supabase is reachable … never a silent half-configured silo (#1/#2/#3)", which the current 4xx-passes behaviour violates.
- **Options:** (A) **tighten the probe** — `return res.status >= 200 && res.status < 400` (a 401/403/404 fails the gate → 503, traffic never routed to a half-config silo) + a test asserting 401 ⇒ not-ok; keep the live smoke as the authoritative key-*validity* check. (B) keep reachability-only + rely solely on the R10 live smoke to catch a bad key (accept that a rotated/typo key deploys green until the smoke runs). (C) two-tier: reachability (2xx-4xx) vs readiness (2xx-3xx) split across `/health` vs `/ready`.
- **Recommendation:** 🟡 **(A)** — low-risk, closes a real #2/#3 "deploys green with a dead DB layer" hole, and aligns the code with its own header contract; a rotated/typo service-role key is a realistic half-config the boot gate should catch loudly, not defer. The live smoke remains the deeper validity proof. Since it changes a documented comment's stated semantics, logged as an OD rather than silently changed.
- **Status:** 🟢 RESOLVED + ✅ IMPLEMENTED (2026-07-08 session 74, operator chose Option A) — `app/service/src/health.ts` `probeSupabase` now returns `res.status >= 200 && res.status < 400`, so a 401/403 (invalid service_role key) or 404 FAILS the boot gate → 503, traffic never routed to a half-config silo (#2/#3). Tests added (`health.test.ts`): 401/403/404 ⇒ not-reachable ⇒ checkHealth not-ok; 2xx/3xx boundary ⇒ reachable. service 9/9, tsc clean. The R10 live smoke remains the deeper read/write-validity proof. See `logic-bug-sweep-findings.2026-07-07.md` (UNCERTAIN).

## OD-196 — write-gate `executeApproved` defense-in-depth: re-read the proposal vs trust the decision object? 🟢 RESOLVED-DIRECTION (seam comment now, harden at ISSUE-056)

- **Surfaced by:** logic-bug sweep (session 74), `app/write-tools/src/write-gate.ts:177` (`executeApproved`), verdict UNCERTAIN (downgraded from the finder's BLOCKER). `executeApproved` takes a caller-supplied `decision: ApprovalDecision`, executes the external write when `decision.status === 'approved'`, and **never re-reads the proposal from the queue nor checks `decision.decidedBy`.** The sole no-self-approval guard (`SelfApprovalRejected` when `decidedBy === AGENT_PROPOSER_ACTOR`) lives only in `InMemoryApprovalQueue.decide()` — so a hypothetical caller that skips `decide()` and hand-forges `{status:'approved', decidedBy:'service_role:agent'}` would self-approve + execute (#2/#6).
- **Why it's a decision, not a live bug:** **no exploiting caller exists** — the only caller is the test (passing a legitimately-decided decision); the real caller is ISSUE-056's approval-queue surface, which is **UNBUILT**. The no-self-approval invariant is **owned by ISSUE-056 by design** (FR-6.APR.005 / AC-6.APR.005.3: reject routing where reviewer == originating_user_id). The intended flow is propose → `decide()` [distinct human; self-approval barred HERE] → the decided proposal drives `executeApproved` (downstream of an already-validated decision). A caller with enough internal access to forge the object could equally call `runtime.invokeWrite()` directly — the gate can't defend against arbitrary internal-object construction.
- **Legitimate kernel:** the self-approval invariant lives in exactly ONE place (`decide()`); `executeApproved` trusts its input with no defense-in-depth if ISSUE-056 is mis-built.
- **Options:** (A) **harden `executeApproved`** — take a `proposalId`, re-read the stored proposal via `deps.approvals.get`, require it exist + be decided-by-a-distinct-human before executing (belt-and-suspenders). (B) leave as-is — the invariant is ISSUE-056's to enforce at `decide()`; document the seam contract that `executeApproved` MUST be fed a `decide()`-produced decision. (C) defer the call until ISSUE-056 is built, then decide with the real caller in hand.
- **Recommendation:** 🟡 **(A) when ISSUE-056 is built** (fold the re-read into that wiring — it's cheap defense-in-depth on a #2/#6 safety surface), OR **(B)** now with an explicit seam-contract comment. Not urgent (no live caller). Kin to OD-194 (both are approval/gate seams awaiting the unbuilt C6/ISSUE-056 caller).
- **Status:** 🟢 RESOLVED-DIRECTION (2026-07-08 session 74, operator chose "seam comment now, harden at ISSUE-056"). ✅ **Seam-contract comment added** to `executeApproved` (`app/write-tools/src/write-gate.ts`): documents that `decision` MUST be a `decide()`-produced decision (no-self-approval barred there), that this method deliberately trusts that upstream decision, and that a caller must never hand-forge an approval + bypass `decide()`. **Owed at ISSUE-056** (the real caller): fold in the belt-and-suspenders hardening (take a proposalId, re-read the stored proposal, require pending + distinct-human approver) — Option A, deferred here because no live caller exists. write-tools 7/7, tsc clean. See `logic-bug-sweep-findings.2026-07-07.md` (UNCERTAIN).

---

## OD-197 — The frontend: how do the specced screens actually get built and tested? 🟢 RESOLVED (operator delegated 2026-07-08)

- **Surfaced by:** operator question mid-Stage-5 build (session 77): *"our issues / entire build plan factor in the screens we need? … we need the app to have screens otherwise how do I test it?"* Investigation confirmed a real structural gap.
- **The gap (facts):** Phase 3 specced **13 surfaces** (`spec/03-surfaces/surface-00…12`, operator-signed-off) and mapped each to an owning issue, and the design doc locks the UI stack (**Next.js App Router + Tailwind + shadcn/ui**, one app per client deployment + a separate super-admin app). **BUT:** (a) every one of the 43+ `app/*` packages built to date is **backend/headless TypeScript** (port + in-memory fake + `supabase-store` adapter + tests) — there is **no `web/`/Next.js scaffold anywhere in the repo**; (b) **no issue scaffolds the Next.js app itself** — `007` provisions infra and `008` is the DB baseline, but there is no UI analog that stands up the app the surfaces render into; (c) the surface issues' §9 nonetheless expects **rendered, tested UI** (component/UI-state tests, false-healthy rendering, reconnect indicators, a11y audit). So the plan describes every screen but never schedules the app that hosts them, nor the render work as distinct from the surface *logic*.
- **Why it matters:** #3-adjacent and product-blocking — a system of headless packages **cannot be exercised or accepted by a human** (the operator's exact point). A whole class of defects (RBAC actually hiding a panel, the never-false-healthy rendering, the logic↔UI seam) only surfaces with real screens on real data. Left unwritten it stays a "we'll do the frontend later" that lives only in chat (Rule 0 violation).
- **Options:** (A) **frontend as a parallel vertical-slice track** — add a *frontend-substrate gate issue* (the UI analog of `008`: Next.js app-shell + Tailwind/shadcn + Supabase auth session + RBAC-driven nav reusing the same PERM nodes `can()` enforces + the data-access seam to the `app/*` packages + local dev harness), then reframe each surface issue to carry two testable layers — the headless **logic** (already being built) + the **React render** (needs the substrate) — and build a *walking skeleton* (auth → Ops dashboard on real data) early so the app is clickable well before the backend is finished. (B) finish all backend stages first, then a big-bang frontend at the end (rejected — leaves the system untestable-by-human for months; defers seam-bug discovery to the worst possible time). (C) pull surface issues from their stages until a substrate exists (rejected — the surface *logic* is worth building now and blocks nothing).
- **Recommendation:** 🟢 **(A)**. The render layer is orthogonal to the backend dependency waves — a screen only needs its *backend signal* `done` (its own dependency rule, mirroring R1), so the frontend can grow as a second lane alongside the remaining stages. Nothing built so far is wasted; the logic is the foundation the pixels sit on.
- **Update — Session 78 (2026-07-08): the SUBSTRATE (ISSUE-087) is BUILT + live-verified → `done`.** The `web/` npm
  workspace now exists — `web/shared` (the skin-swappable design system: tokens-as-CSS-vars + semantic components + the
  RBAC-nav gate + honest-state logic + the data-access seam; 11/11 offline tests reusing the real `@harness/rbac`),
  `web/rbac-bridge` (the one seam into `app/rbac`), and the two Next.js apps `web/client` + `web/admin` (ADR-001 §7), both
  booting + typechecking clean. The **walking skeleton is now achievable** (auth → shell → live signal is clickable). Built
  skin-swappable exactly as this OD's binding constraint requires (dark theme is a pure token swap — verified). **Still owed
  (unchanged):** the per-surface **render** decomposition (`to-issues` pass) now that `087` has landed; the real-Supabase
  live-auth close (ISSUE-013 / OD-175). See ISSUE-087 §10 for the full evidence.
- **Status:** 🟢 RESOLVED (2026-07-08, operator "do what you think is best" → Option A). **Actions taken this session:** (1) this OD; (2) **[[ISSUE-087]]** created — the frontend-substrate gate (Next.js app-shell + super-admin app, auth, RBAC nav, data-access seam, honest-state UI primitives, a11y baseline); (3) BUILD-SCHEDULE gains a **Frontend track** section (substrate → walking skeleton → per-surface render, each render gated on its backend signal `done`); (4) a **clickable surface-05 prototype** built + shared with the operator (Artifact) as the first "see it" proof. **Owed (explicitly, not dropped):** the per-surface **render** decomposition — each surface issue (`013`/`021`/`026`/`031`/`056`/`067`/`072`/`073`/`078`/`079`/`086`) reframed to carry a render sub-deliverable, or new render issues minted (a `to-issues` pass, once `087` lands). The Stage-5 surface members building now (`021`/`078`/`079`/`086`) close as **logic-done, render-pending** against this OD — not as full screens.
- **Render sequencing (operator decision, session 77):** build **all 13 spec surfaces first with a clean, spec-faithful default UI**, let the operator see the whole working app, **then retrofit the operator's own HTML dashboard design last** ("I want to see everything you build, then we retrofit — cleaner"). **Binding build constraint this creates:** the default render layer MUST be built **skin-swappable** — design **tokens** (color/type/spacing as CSS variables) + semantic components, **no hardcoded styling in markup** — so the operator's later retrofit is a **reskin, not a rebuild** (the expensive layer — component structure, the data-access seam wiring, RBAC gating, honest-state logic — is built once; only the skin swaps). The operator's design informs a `to-issues` restyle pass after the full default UI is visible; where his design implies features the backend lacks, spec them as new FRs/issues (Rule 0), not silent bolt-ons. **Operator expectation (session 77, explicit): the retrofit is NOT expected to be a pure reskin — his dashboard likely carries features the design-doc-v4-derived spec omits. So the retrofit phase = (a) restyle over the built structure + (b) a fresh requirements pass over the operator's dashboard to identify + spec the missing features as new FRs/surfaces/issues (some UI-only, some needing new backend). His HTML dashboard is treated as a first-class source of evolved intent, reconciled against the built app once it exists — which is precisely why the build-everything-first-then-review sequencing was chosen (gaps are visible against a running app, not against a plan).**

---

## OD-198 — Stage-5 offline-batch close: the forks the adversarial verify + fix pass surfaced 🟡 (mixed — several fail-safe-shipped, a few need a call) (session 77, 2026-07-08)

- **Surfaced by:** the ISSUE-021/052/058/062/065/068/078/079/086 fan-out build → adversarial verify (2 BLOCKER + 7 MAJOR + 19 MINOR) → fix pass. All BLOCKER/MAJOR + cheap MINORs FIXED fail-safe with regression tests; the items below are the residual design/spec forks the fixers were told NOT to guess. **Every one already ships fail-safe behaviour** — these decide durability/consolidation/reconciliation, none is a live hazard now. Grouped here (not 12 separate ODs) as batch-close forks; promote any to its own OD if it needs independent tracking.
- **① [021-a] Cross-package last-Super-Admin advisory-lock key — 🟢 RESOLVED this session.** The user-mgmt fix chose key `0x55410005` but `@harness/rbac` (the homed owner, ISSUE-018) locks on `pg_advisory_xact_lock(hashtext('rbac:super_admin_guard'))` — different keys ⇒ a demotion via rbac and a deactivation via user-mgmt would NOT serialize (write-skew, last-SA orphaned, #2). **Fixed:** user-mgmt adopts rbac's exact key verbatim. **Owed (doc, Rule 0):** record in ADR-004 that EVERY SA-guard mutation across ALL adapters must take `pg_advisory_xact_lock(hashtext('rbac:super_admin_guard'))`, and make it a named shared constant so it can't drift.
- **② [062] `hard_limit_class` tool tag — home + mandatory seed. 🟡 needs a call.** The "Memory is sole memory-writer" guard (AC-8.SPC.005.2) keys on a `tools.config.hard_limit_class` tag (`memory_write`|`autonomous_send`|`transaction`) that **no migration/C3/tool ships yet**. Fix shipped: classifier is now **FAIL-CLOSED** — an untagged write-category tool is DENIED to every role (incl. Memory) until tagged. **Consequence:** the memory-write tool's seed tag is now MANDATORY (Memory can't get its writer tool until seeded). **Recommend:** (a) document the tag convention in `schema.md §4 (tools)` + component-03; (b) seed the memory-write tool's `config.hard_limit_class='memory_write'` at provisioning (a seed/migration, owed to C3/ISSUE-007-provisioning path). Confirm fail-closed is the intended posture (it is the #2-safe one).
- **③ [079] Human-path RLS gap on the surfaces' READ tables — 🟡 the notable one.** `push_subscriptions` fixed here (migration 0033, owner-scoped + aal2). But `task_queue`, `notifications`, `event_log`, `conversations`, `messages`, `proactive_suggestions`, `guardrail_log` are all **RLS-enabled + REVOKE-ALL + NO authenticated policy** (0001c default-deny, never re-opened for the human path) — so the mobile/ops surfaces' reads return **0 rows on the human path live** (fail-CLOSED, no leak — good for #2 — but renders a false-healthy "0"/"all-clear" and the surface is non-functional for a logged-in user). These read policies are **owed to the PRODUCER issues** (C5/C6/C7/C9 — ISSUE-075/076/056/052/…), not to ISSUE-079. **Recommend:** producer issues add the aal2-gated human-path read policies (recipient/originating_user_id/clearance-scoped); AND the surface read layer must distinguish "authorization returned nothing" from "genuinely zero" so an RLS-denied read never renders all-clear (never-false-healthy-on-empty, #3) — owed alongside those policies. **Reconcile before any of these surfaces is called live-verified.**
- **④ [068] config-registry vs FR conflict (autonomy sub-type editability). 🟡 reconcile.** `config-registry.md L331 (§I item 9)` says ALL FIVE action-autonomy sub-types render read-only / edits rejected; `FR-9.MODE.004.1` says a Super Admin CAN edit `low_risk_external_nonclient` within ≤Prepare. Package built to the **FR** (fail-safe: only that one sub-type editable within {suggest,prepare}; the 4 floored sub-types reject any edit). **Recommend:** FR wins (Rule 0 — the FR is the requirement, the registry is derived) → correct `config-registry.md L331`.
- **⑤ [058-a] `max_retries_before_dead_letter` ceiling. 🟡 trivial reconcile.** Code enforces a finite ceiling of 20 (AC-6.RTL.001.1 "meaningful finite ceiling"); `config-registry.md L189` says `int ≥ 0` (unbounded). Fail-safe shipped (validator rejects >20). **Recommend:** tighten the registry range to `int 0–20`.
- **⑥ [058-b] Log-write-failure posture. 🟡 ratify.** Kept the ISSUE-055 house style (return the safety decision + surface `logWriteFailed` + console.error, don't hard-throw) so a throttle/kill still reaches C5 if the audit write hiccups. Residual #1/#3 if C5 ignores `logWriteFailed`. **Recommend:** ratify surface-and-continue as the consistent C6 posture (or reverse all C6 slices to hard-throw) — one OD, applied uniformly.
- **⑦ [065] Producer-heartbeat granularity. 🟡 defer-with-DDL-ready.** AC-8.HLTH.004.2 names THREE independently-stallable producers but `agent_health_metrics` ships ONE shared `producer_heartbeat` column — one running cycle marks all three "fresh", so a stalled routing-mismatch producer (LRN.002, ISSUE-066) reads fresh (#3). **Recommend:** a `agent_health_producer_heartbeats(agent_id, producer, beat_at)` table (DDL drafted by the fixer, NOT applied) — decide when ISSUE-066 lands; interim behaviour disclosed, not silent.
- **⑧ [086] SECRET/hard-limit DB backstop. 🟡 defense-in-depth.** SECRET-key + locked/hard-limit write rejection is app-layer only (fake + adapter + pre-screen). A raw-SQL/service_role write bypassing the adapter has NO DB backstop. **Recommend:** a CHECK/BEFORE-INSERT trigger on `config_values`/`config_audit_log` rejecting SECRET-class keys (fixed enumeration → feasible), owned by ISSUE-010/the schema owner.
- **⑨ Lower-stakes confirmations (fail-safe shipped, note-and-move):** [021-b] `isOAuthUser` provider inference — verify against OD-018 seeding at the R10 live run. [078] `DeploymentSnapshot` type has no compile-time link to ISSUE-012's producer (cross-package contract-test/shared-types decision, maintenance-time #3). [086-b] `alert_engine_stalled` deliberately not in the routed CRITICAL set (surfaced via the always-loud banner instead) — confirm posture. [062-MINOR] specialist rejection durable-audit is an optional dep with no DB table (deferred). [065-MINOR] `loadScope` memory_scope jsonb shape deferred to ISSUE-063/OD-177.
- **Status:** 🟡 recommendations pending operator's call on ②/③/④/⑥ (①/⑤ are low-risk applies; ⑦/⑧/⑨ owed to their owning issues). **No blocker to committing the batch code** (all fail-safe). **Blocks flipping the affected issues fully `done`:** the R10 live smokes (owed) + ③'s producer-RLS reconcile for the surface issues (079/078). The non-surface logic issues (052/058/062/065/068 + 021) can close on their own AC + live smoke; the surface issues (078/079/086) close as **logic-done, render-pending** ([[OD-197]]) with ③ tracked.
...
- **⚠️ Session-79 reality update (load-bearing — the framing above assumed the connectors were offline-built; they are NOT):** as of Session 79, `039`/`040`/`041` remain **`ready` and entirely UNBUILT** (no `app/<slug>/` package exists — only the shared `app/connector-runtime` from Stage 3). The Session-79 overnight batch built `038`/`064`/`083` (now `in-progress`) but deliberately did **not** start the connectors' offline logic (the "if time" stretch was not reached — see SESSION-LOG Session 79). So Option (A) as originally worded ("connectors close on their **offline AC + logic**") presumes a build that does not exist. The close session must therefore choose between **two concrete branches for the connectors, both of which the operator can ratify without further ambiguity:**
  - **(A-defer) Close Checkpoint 5 with `039`/`040`/`041` deferred UNBUILT to a later session / onboarding** — treat them exactly like any onboarding-gated vendor work (OD-172: the operator has **no GHL account** and cannot exercise any connector live regardless, so building offline-only logic now vs. next session does **not** change the checkpoint's live-provability — it is onboarding-gated either way). The checkpoint ticks on the **built** batch members (`022`/`020`/`021`/`052`/`058`/`062`/`065`/`068` `done` + `038`/`064`/`083` flipped `done` this pass) proving as a group; the connectors + their live arm are a tracked residual carried into Stage 6, not a checkpoint blocker. **🟢 RECOMMENDED** — it is the Stage-3/4 precedent applied honestly (Stage 3 closed with `013`/`014`/`033` live arms owed; Stage 4 closed with per-vendor arms AF-090/084/083 owed) and unblocks Stage 6 on genuinely-done work, without pretending unbuilt code exists.
  - **(A-build) Build `039`/`040`/`041` offline first** (tool-row registration + param wiring + reuse `@harness/webhook-auth`, dossiers current) **then** close per the original Option (A). Correct but strictly slower, and it does not improve live-provability (still onboarding-gated). Choose this only if you want the connector *offline logic* in-repo before Stage 6 opens.
- **Options:** (A) **Extend the Stage-3/4 precedent:** connectors close on their offline AC + logic (live vendor arm = onboarding residual, OD-172); surfaces close as `done` on their logic AC + R10 smoke with render tracked as Frontend-track render issues (OD-197) and OD-198 ③ reconciled — then the Checkpoint-5 integration test proves the *group* and the checkpoint ticks. **(Superseded by the A-defer / A-build split above given the connectors are unbuilt.)** (B) **Hold the checkpoint** until connectors' live arms + the three surfaces' render all land (rejected-leaning: leaves Stage 6 blocked for months on onboarding-gated vendor infra the operator can't even exercise yet). (C) split: connectors per (A), surfaces held.
- **Surfaces `078`/`079`/`086` (part b):** close as `done` on their logic AC + R10 smoke (078/079 already R10-green, session 77) with **render** tracked as Frontend-track work (OD-197). **⚠️ prerequisite for calling `078`/`079` *live-verified* at close:** reconcile **[[OD-198]] ③** (the task_queue/notifications/event_log human-path RLS gap — their reads return false-healthy 0-rows live until the producer issues add authenticated policies); until then, close them as *logic-done, render+live-read-pending* rather than fully live-verified. `086` is logic-done, render-pending.
- **Recommendation:** 🟢 **(A-defer)** for the connectors + close the surfaces as logic-done (render = Frontend track) with OD-198 ③ noted. This keeps every #1/#2/#3 guarantee (everything shipped fail-safe; every deferred arm is *tracked, not silent*) and unblocks Stage 6. **This is a scope call the operator ratifies at the close session** — pick A-defer or A-build for the connectors, decide the surfaces, then write the chosen rule here (flip this OD 🟢 RESOLVED) so R4/checkpoint-close is unambiguous going forward.
- **Status:** 🟡 OPEN — a framing OD for the Checkpoint-5-close session, now with both connector branches spelled out (A-defer recommended). Decide the connectors (A-defer / A-build) + the surfaces explicitly before ticking the checkpoint. Does not block flipping `038`/`064`/`083` `done` (all fully specced, no vendor gate).

---

## OD-200 — How does the C5 task_queue represent a connector-disconnection-paused task? 🟡 OPEN (surfaced building ISSUE-038, Session 79)

- **Surfaced by:** building ISSUE-038 (connector disconnection + recovery). §8 step 1 says *"confirm ISSUE-048's `task_queue` has a pause/resume status transition … if any is absent, flag it as an upstream gap — do not silently add it here."* Verified first-hand: `task_queue.status` (enum `task_status`) has **no `paused` value** — its states are `pending / running / awaiting_approval / completed / failed / flagged`, and `flagged` (the only "held" state) leaves ONLY by an explicit human review action (requeue/discard/approve), so it is **not auto-resumable** on reconnect. There is no status a disconnection can move a task into that C5's dispatcher will (a) skip while paused and (b) auto-resume on reconnect.
- **What ISSUE-038 did (fail-safe, no guess):** the DSC slice owns the **durable paused-SET** (migration `0034` `connector_disconnection_paused_tasks`) + drives resume through an injected re-check seam, so a paused task is **never silently abandoned** (AC-3.DSC.003.3 holds) regardless of how C5 represents the status. It did **not** invent a `paused` task_status (that is ISSUE-048/052's enum to own — a silent add here would be the exact drift Rule 0 forbids).
- **The question (for C5 / ISSUE-048/052 + the Checkpoint-5 integration):** how does the task_queue status machine represent "paused by a connector disconnection"? Options: (A) **add a `paused` task_status** (+ the allowed transitions `running→paused`, `paused→running`) that the dispatcher skips and the DSC resume flips back — the cleanest, an additive enum migration owned by ISSUE-048; (B) **no new status** — the dispatcher consults the DSC paused-set (0034) as a skip-list and the task stays `pending`/`running` but un-dispatched (couples the dispatcher to the DSC table); (C) **reuse `flagged`** — rejected (flagged requires human review to leave, so auto-resume would violate FR-5.QUE.003.2). 
- **Recommendation:** 🟢-leaning **(A)** — an additive `paused` task_status is the least-coupled, most legible representation and matches the FR-3.DSC.003 "pause … auto-resume" language. But it is **ISSUE-048/052's enum to author** (C5), not ISSUE-038's, so it is logged here for that slice + the Checkpoint-5 integration to ratify. Does **not** block ISSUE-038 closing (the durable set + re-check seam make it fail-safe today).
- **Status:** 🟡 OPEN — a coupling fork for C5. Everything ships fail-safe (#1/#2/#3): no task is abandoned, the paused set is durable, resume re-checks authz. **Trigger re-pointed (Session 80 — Checkpoint 5 closed without deciding this; it did not need to):** decide (A/B/C) **when ISSUE-048/052's dispatcher first wires the DSC pause live** (i.e. when a connector-disconnection actually pauses/resumes a real dispatched task) — that is the moment the `paused` status representation must exist. Not before; the durable paused-set makes ISSUE-038 fail-safe today.

## OD-201 — `step_failure_mode` taxonomy drift: orchestrator shorthand vs the DB enum 🟡 OPEN (surfaced building ISSUE-064, Session 79)

- **Surfaced by:** building ISSUE-064 (execution plans + failure-mode). The orchestrator (ISSUE-061, `app/orchestrator/src/routing.ts`) emits `STEP_FAILURE_MODES = ['halt_escalate','retry','skip']` on `PlanStep.failure_mode`, but the **DB enum** `step_failure_mode` (baseline 0001) is `('retry','skip_and_continue','halt_and_escalate')`. The strings differ (`halt_escalate` ≠ `halt_and_escalate`; `skip` ≠ `skip_and_continue`). `step_failure_mode` is a **documentation enum typed inside `plan_body`/`steps` jsonb**, not a column CHECK, so the DB does **not** reject a raw shorthand value — it persists silently, and any consumer reading `plan_body.failure_mode` (ISSUE-067 history UI, analytics grouping by mode) would see mixed vocabulary in one field (a silent #3 drift).
- **What ISSUE-064 did (fail-safe):** ISSUE-064 **owns the taxonomy** (per the routing.ts co-ownership comment). It defines the CANONICAL set = the DB enum values, and `toCanonicalFailureMode` maps the orchestrator shorthand → canonical. Its `assignFailureModes` canonicalizes on the way in, its live `saveVersion` canonicalizes + asserts at the **write boundary** (so `plan_body` always stores canonical values regardless of caller), and `readPreAssignedMode` maps either form on read — so ISSUE-064's own paths never emit shorthand.
- **The residual (the real fix, owed to ISSUE-061):** the orchestrator's **own** routing-time `plans.saveVersion(buildPlan(...))` path (routing.ts:328, `failure_mode:'halt_escalate'`) does **not** route through ISSUE-064's canonicalizer, so a plan persisted at ORC.007 time (not via ISSUE-064) still writes shorthand into `plan_body`. The clean fix is for 061's `buildPlan` to emit the canonical values (or call `toCanonicalFailureMode`) — a small, change-controlled edit to ISSUE-061 (which is `done`), owed at the Checkpoint-5 reconcile.
- **Recommendation:** 🟢 have ISSUE-061's `buildPlan` emit canonical `step_failure_mode` values (align the `STEP_FAILURE_MODES` constant to `('retry','skip_and_continue','halt_and_escalate')` and update the two `failure_mode:` literals). Until then ISSUE-064's write-boundary canonicalize+assert absorbs the drift for any plan flowing through it. Everything is #3-safe (no wrong-mode execution — the canonicalizer + halt-and-escalate default cover unknown values).
- **Status:** 🟡 OPEN — a small change-controlled edit owed to ISSUE-061's `buildPlan`. **Trigger re-pointed (Session 80 — the Checkpoint-5 reconcile passed without making this edit; it is not urgent because ISSUE-064's write boundary canonicalizes):** make the edit **before/when ISSUE-067 (agent-builder surface, now `ready`) — or any consumer — reads `plan_body.failure_mode` live** (that is the first place raw shorthand persisted by 061's own routing-time `saveVersion` would be visible as mixed vocabulary). Does not block ISSUE-064 (its own paths are canonical + write-boundary-guarded). **Session-83 update (ISSUE-067 built):** ISSUE-067's Execution-Plans section renders `plan_body.failure_mode`, but on the **dev-auth/SEEDED seam** (its seeded plans carry canonical values) — it does **NOT** read live orchestrator-persisted plans yet, so the mixed-vocabulary drift is still **not exposed**. The fix stays deferred (editing the locked ISSUE-061 core constant mid-batch risks cascading its 40-test suite for a drift nothing live currently sees) — the true trigger is when 067 (or analytics) reads **live** `plan_body.failure_mode` on real orchestrator-persisted plans (the same "deferred until live data" pattern as OD-198③). Still 🟡 OPEN.

---

## OD-202 — AC-8.SCO.003.1 `memory_scope` shape validation: Builder front gate only, no SERVER write twin 🟡 OPEN (surfaced verifying ISSUE-067, Session 83, 2026-07-10)

- **Surfaced by:** the independent adversarial verify of ISSUE-067. AC-8.SCO.003.1 ("an invalid `memory_scope` edit is rejected at write") is enforced by the Builder **front gate** — `web/agent-bridge/src/builder-guard.ts` `validateMemoryScope` (fail-closed shape validation, wired into `evaluateBuilderSave`, proven by tests + live in-browser). But ISSUE-061's **server write path** (`app/orchestrator/src/registry.ts` `validateWrite`/`editCapability`) rejects only `memory_scope === null`, **not a malformed-shape scope**, and the DB column `agents.memory_scope jsonb NOT NULL` has **no CHECK**. So a write that BYPASSES the Builder (a direct service_role / API write) could persist a structurally-malformed scope.
- **Why it's fail-safe TODAY (not a live #2 breach):** (a) the Builder is the only HUMAN edit path and it validates; (b) `memory_scope` is **not consumed yet** — the per-agent scope *retrieval filter* is ISSUE-063 (Stage 8, unbuilt), and the canonical roster is seeded with the fail-closed narrow `'{}'` (OD-177) which retrieves NOTHING; (c) a malformed scope is a broken least-privilege FILTER (retrieves less, fail-closed-narrow), not an over-grant. The tool-capability hard limits (SPC.003.3/.004.3/.005.2) — the actual #2 containment — ARE enforced server-side by ISSUE-062's code (`evaluateLiveGrant`, AF-068 GREEN), independent of this.
- **The fix (owed to ISSUE-063 / the next orchestrator touch):** lift `validateMemoryScope` into a shared leaf (or `app/orchestrator/src/registry.ts`) that BOTH the Builder bridge guard AND the server write path (`validateWrite` + `editCapability`, live adapter included) import — ONE validator — so a non-Builder write cannot persist a malformed scope; optionally add a DB CHECK. Change-controlled (ISSUE-061 is `done`).
- **Recommendation:** 🟢 compose the shared `validateMemoryScope` into ISSUE-061's server write path when **ISSUE-063 wires the real per-agent scope filter** (the point the column first becomes load-bearing) — before any live retrieval trusts `agents.memory_scope`. Until then the Builder front gate + the fail-closed-narrow seed keep it safe. ISSUE-067 ships `done` on the surface-level AC (the Builder save path IS the first of the three defense-in-depth layers per §9); this OD tracks the server twin.
- **Status:** 🟡 OPEN — owed at ISSUE-063 (or the next orchestrator change-control touch). Does not block ISSUE-067 or Checkpoint 6.

---

## OD-203 — Model-call SDK layer: Vercel AI SDK (design-doc primary) vs direct `@anthropic-ai/sdk` + `openai` (ADR-009 implicit) 🟢 RESOLVED → ADR-012 (operator chose the Vercel AI SDK, 2026-07-10)
...
- **Resolution (2026-07-10, operator):** chose **(A) — the Vercel AI SDK** (design-doc intent). The model-call layer is the **Vercel AI SDK as the primary unified interface + the Anthropic SDK alongside** for Claude-specific features (extended thinking, citations); `openai` is retained for **embeddings** only. Reason: per-task model routing / provider-swap as a first-class abstraction is load-bearing for the C8 orchestrator routing + the cost ladder, and adopting it now (before any real model adapter exists) costs nothing already shipped. Recorded via change-control as **[[ADR-012]]**, which supersedes the **model-call SDK-layer portion of ADR-009 only** (ADR-009's TypeScript/Node language decision is unchanged). The ISSUE-001 cost spike's direct-SDK usage is historical/throwaway and left as-is (AF-001 economics unchanged — the AI SDK is a thin layer over the same provider APIs). **Verification owed (not yet proven):** when the first real model-client adapter is built (ISSUE-053 / orchestrator live path), confirm the AI SDK cleanly expresses the Anthropic-specific escape-hatch calls + the cost-ladder model-downgrade routing (flagged in ADR-012; no new AF minted).
- **Status:** 🟢 RESOLVED → ADR-012. Unblocks the model-client adapter target for ISSUE-053 / the orchestrator live path.

---

## OD-204 — ISSUE-029 transitive erasure has no queryable derived-row provenance to walk (merge/summary "derived_from" is written only to `event_log`, not to `memories`) 🟢 RESOLVED (Session 87, option A) — provenance edge now persisted
...
- **Resolution (Session 87, option A taken):** migration **`0045_memories_derived_from.sql`** adds `memories.derived_from uuid[]` (nullable; null = a directly-written, non-derived row) + a GIN index `memories_derived_from_gin` for the containment/overlap lookup. ISSUE-027's live adapter (`app/memory-maintenance/src/supabase-store.ts` `insertDerivedMemory`) now persists the already-computed source set into that column (`$15::uuid[]`, `null` for the empty/feedback direct-write path); the InMemory fake already tracked it in its `derivedFrom` Map, so offline/live parity holds. ISSUE-027's `check` gate gained a corpus-wide verify-present for the column, and two adapter tests assert the write (derived → source ids; direct → null). 027 stays `done` — this is purely additive persistence of an edge it already computed (no behaviour change). ISSUE-029's transitive walk queries `derived_from && ARRAY[<erased ids>]` to reach summary/merge rows. 027 tests 40 → 41, all green; typecheck clean.
- **Status:** 🟢 RESOLVED (Session 87). Unblocks: the ISSUE-029 transitive-walk design + AF-137.
