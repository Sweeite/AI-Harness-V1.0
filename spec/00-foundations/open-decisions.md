...
- **Status:** 🟢 RESOLVED (2026-07-07, operator delegated to recommendation) — lifecycle IS v1 (the methods are specced FR-0.INV.003/.005/.007 and built) → **model it on the existing pending-`profiles` row** (revoke = deactivate the pending profile + audit; reissue/resend = re-mint + re-send; markBounced = flag + audit); no new table (Option A). **Immediate sub-fix (owed):** stop the silent delegate-to-empty-fake — the 4 live methods must operate on `profiles` or throw not-implemented until wired (never a silently-wrong result, #3). Full implementation owed (fresh chat). See `live-adapter-backfill-findings.2026-07-07.md` (B5).
