...
|---|---|---|---|
| **1 · Never lose or corrupt knowledge** | memory integrity — nothing silently dropped, overwritten, or scrambled | supersede-not-delete · contradiction check · idempotency + per-entity serialize (ADR-004) · shadow-retain keeps a would-drop (ADR-003) · **backup/DR: hourly client-owned off-platform snapshot (PITR upsell) + tested restore (ADR-008)** · **golden rule — source data re-derivable from systems of record (`L1634`)** · dims #2,#3 | ⚠️ **AF-069** — the *restore* must be proven to work (a backup you've never restored is a guess); ADR-008 decided the posture, this is the paper-until-proven residual |
...
| 11 | **Backup / disaster recovery** | tested restore · defined ownership under client-owned Supabase | hourly client-owned off-platform snapshot (PITR upsell) + operator-verified restore rehearsal + backup-health on the mgmt-plane push (ADR-008) | 🔵 + ⚠️ AF-069/070/071/072 |
