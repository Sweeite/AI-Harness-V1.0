...
9. **`action_autonomy_matrix`** — *how freely the AI may act on its own for each kind of action (sensitive ones are locked to needing sign-off).* **all sub-types LOCKED at hard-approval-or-Prepare, no exceptions (OD-161, 2026-07-02 — reverted the prior Act-tier carve-out for `low_risk_external_nonclient`):** `low_risk_external_nonclient` · `existing_client_external` · `system_of_record_comms` · `financial_operation` · `confidential_restricted_action` all render read-only with a "Locked (hard_approval-or-Prepare)" badge; **edits rejected at write** (AC-9.MODE.004.2). The retired `act_requires_trust_period`/`act_trust_period_days`/`act_rate_cap_per_hour` fields no longer exist on any sub-type.
10. **`price_table`** — *the estimated cost per use of each AI model, used for budgeting.* vendor×model→{input, output} $/1k tokens (+ embedding $/unit), estimate-grade, operator-editable; e.g. anthropic: opus 0.015/0.045 · sonnet 0.003/0.015 · haiku 0.0008/0.004; openai embedding `text-embedding-3-small` **0.00002 $/1k input** (= $0.02/1M, standard tier — **not** the $0.01/1M batch rate; the round-up estimator forbids optimistic batch discounts). *Embedding rate filled from OpenAI primary-source pricing, verified 2026-07-03 during the ISSUE-001 cost spike; re-verify if the vendor rate moves.* Floats ≥ 0.
