...
| `action_autonomy_matrix` | `config_values.value` WHERE key = 'action_autonomy_matrix' | Structured sub-table; LIVE; **two sub-sections**: (1) Configurable: `low_risk_external_nonclient` {default_mode, act_requires_trust_period, act_trust_period_days, act_rate_cap_per_hour}; (2) LOCKED floored rows (existing_client_external · system_of_record_comms · financial_operation · confidential_restricted_action) — rendered read-only with a "Locked (hard_approval)" badge; writes to floored rows rejected at server; requires `PERM-guardrail.edit_autonomy` for any edit to the configurable row |
| Last updated / actor | `config_values.updated_at`, `config_values.updated_by` | Shown per row |
...
| `external_act_trust_period` | `config_values.value` WHERE key = 'external_act_trust_period' | int days ≥ 0; BOOT |
