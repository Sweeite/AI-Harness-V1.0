...
| ISSUE-025 ✅ **done** | Retrieval + ranking + clearance-before-ranking + answer modes — **DONE (session 85, 2026-07-10):** `app/retrieval/` (port + fake + `supabase-store.ts` + check + **54/54** + tsc); the Stage-7 GATE. Clearance-BEFORE-ranking realises the live `0031` predicate in code (agent path bypasses RLS → retrieval is the authoritative #2 filter) + OD-081 agent-scope; OD-169 ranking; Restricted-never-inject; answer-mode/[Building] signals. Read-path only (no migration); **R10 smoke 13/13 PASSED** vs live silo (rolled back, silo clean). 5-lens adversarial verify: **clearance-leak lens EMPTY**; 1 MAJOR (two-clock arm divergence → arms now RAW, pipeline single filter authority) + 3 MINOR/NIT fixed. Ship gates 🟢 AF-067/🟢 AF-019; carries 🔴 AF-002 (real-corpus recall). GitHub #25 CLOSED. | C2 RET | 023, 020 |
| ISSUE-026 ✅ **done** | Ingestion filters + human queue — **DONE (S85):** `app/ingestion/` 59/59 + R10 batch smoke PASS (0041 LIVE); no-backdoor invariant in code; adversarial-verify 1 BLOCKER + 2 MAJOR fixed (non-committed-write no longer marks items done/lost). AF-043 carried. #26 CLOSED. | C2 ING | 024, 032 |
| ISSUE-027 ✅ **done** | Maintenance lifecycle — **DONE (S85):** `app/memory-maintenance/` 40/40 + R10 batch smoke PASS (0042 LIVE); adversarial-verify 2 MAJOR fixed (supersede/merge now freeze under-review slots; escalated conflicts included). #27 CLOSED. | C2 MNT(lifecycle) | 024 |
| ISSUE-028 ✅ **done** | Conflict quarantine + consolidation approval — **DONE (S86):** `app/conflict-consolidation/` 51/51 + check green + R10 live smoke 15/15 PASS (0044 LIVE, head `0044`). FR-2.MNT.008 5-rule resolver + Keep-new/existing/both (routed through the sole writer, re-embeds the held draft, non-commit surfaces loudly #3) + Personal/Restricted consolidation gate (skip-by-default) + two-queue escalation. Adversarial-verify 1 MAJOR (Personal-gate scalar-tier bypass → gates full tier set) + 4 MINOR + 1 NIT fixed. AF-061 🟡 consumed. #28 CLOSED. Leaf. | C2 MNT(conflict/consol), WRT.002 | 024, 056 |
| ISSUE-029 ✅ **done** | Compliance erasure walk (memory-side transitive delete) — **DONE (session 87):** `app/memory-erasure/` (port+fake+live adapter) — the ONE sanctioned destructive path: gate → transitive walk (superseded chain both ways + OD-204 `derived_from` edge) → classify (derived/single-entity→delete; multi-entity→retain+scrub; other-subject→exclude+relink-live) → sole `delete from memories` → immutable tombstone → raise backup-purge flag + trigger C7 log redaction → verified-complete-or-fails-loud incl. independent residue re-read. **AF-137 GREEN** (residue spike 21 assertions). Migrations `0045` (OD-204 `derived_from`) + `0046` (2 erasure event_types) LIVE (head 0046). 47/47 offline + R10 16/16 PASS. 3-lens verify: 1 BLOCKER (over-erasure) + 2 MAJOR + MINOR/NITs fixed. OD-204 RESOLVED. GitHub #29 CLOSED. Unblocks 082. | C2 MNT.017 | 024; → 082 |
...
| ISSUE-066 ✅ **done** | Orchestrator learning + cache + cost — **DONE (S85):** `app/learning-cache-cost/` 33/33 + R10 batch smoke PASS (0043 LIVE); cache put now per-key advisory-lock atomic; planOutcomes-vs-064 MINOR carried. #66 CLOSED. | C8 LRN, COST | 061, 074 |
...
| ISSUE-082 ✅ **done** | Individual right-to-erasure workflow (two-person auth, verify-before-done) — **DONE (session 88):** `app/compliance-erasure/` (port+fakes+live adapter) — the C10 request-to-audit workflow wrapping ISSUE-029's C2 `eraseTarget`: Admin queue + escalation; two-class identification (deterministic vs recall-oriented name-in-content, surfaced-for-confirmation; AF-134 EVAL green); **two-person auth as a perm-checked persisted handshake** (execute reads persisted authorisers, two-person unconditional/DB-mandated); frozen guard; de-link + narrow-term `[REDACTED]` scrub via sole-writer; connector flags; **verify-before-done ALLOWLIST** (empty/failed/blocked/unknown → hold+escalate); immutable audit fail-closed. Migrations `0047` (9 lifecycle event_types) + `0048` **contract** (fixed a latent 0001 `deletion_requests` distinctness-CHECK bug found by R10) LIVE (head `0048`). 46/46 offline + R10 15/15 PASS. 3-lens verify: 1 BLOCKER + 2 MAJOR + MINOR/NITs fixed. OD-206 logged. GitHub #82 CLOSED. Leaf. | C10 DEL | 029, 021 |
