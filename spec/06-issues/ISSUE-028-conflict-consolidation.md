...
1. **Migration** — add the `memory_conflicts` + `consolidation_approvals` tables and the `mem_review_state` / `consolidation_op` enums per `schema.md` §3 (expand-only; ISSUE-008 harness). No `client_slug` (ADR-001 §3).
