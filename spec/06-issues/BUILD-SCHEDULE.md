...
they'd both grab `0003` and collide on `_journal.json`. **Applied-LIVE head: `0030_entity_types_config_seed` (session 75, ISSUE-022).
Next free silo tag is `0031`.** *(Session-74 added `0027_profiles_invite_lifecycle` [OD-192 invite lifecycle] + `0028_task_queue_awaiting_approval_at`
