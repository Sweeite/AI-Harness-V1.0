...
they'd both grab `0003` and collide on `_journal.json`. **Current head (applied LIVE, session 69): `0010_guardrail_escalation_nullfix` → next
free tag is `0011`.** *(Stage-3 authored `0006_profiles_owner_rls` [013], `0007_stage3_event_types` [013+047, 9 additive
...
### Stage 4 — Behaviour on the models  *(OPEN since 2026-07-06 — Checkpoint 3 CLOSED; gate `019` + all 14 batch members `ready`)*  *(14 in parallel)*
- [ ] 🟠 **GATE — `019` Clearance + Restricted model** `ready` (blocked-by `018` ✅) — every memory tag & RLS predicate above reads this. **Build serial + hardest first (R3).**
