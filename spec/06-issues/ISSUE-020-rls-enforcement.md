...
**Found → FIXED (ISSUE-013 latent gap):** `profiles` had NO `authenticated` SELECT grant (0006/ISSUE-013 never granted it after 0001c's revoke), so `profiles_owner_read`/`_update` were dead (permission-denied) — fail-CLOSED (no leak). Fixed at the operator's request in **migration `0032_profiles_authenticated_grant`** (`grant select` + column-scoped `grant update (name)`; applied LIVE, head `0031`→`0032`) — see ISSUE-013's post-build-fix note.
