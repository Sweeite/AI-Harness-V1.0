...
- **Acceptance criteria:**
  - AC-NFR-COST.006.1 — Given the AF-001 spike over a real task + memory write, When typical-volume cost is measured, Then it lands at or below ~$20/day and under the $50/day soft alert. — ✅ **Verified 2026-07-03** (AF-001 PASS: **$2.09/day** extrapolated, round-up all-vendor estimator).
  - AC-NFR-COST.006.2 — Given AF-001 measures typical volume above the soft alert, When the team responds, Then the cost levers are pulled in the COST.007 order before the ceiling is raised. — ✅ **Verified 2026-07-03 (vacuously)**: measured typical was under the soft alert, so no lever pull was required; the over-soft-alert path is implemented in the harness (`extrapolate.ts`/`thresholds.ts`) and asserted by dry-path.
- **Verification result (AF-001, 2026-07-03):** SPIKE+EVAL PASS. One real multi-agent task (orchestrator→research→2 specialists→synthesis) = $0.0359; one surviving memory write = $0.0025 (1 Sonnet + 3 Haiku + 1 embed, ADR-003 §4 shape confirmed); non-survivor = 0 Sonnet. Extrapolated against the declared profile (50 tasks/day · 500 write-events, 100 survive · 169 idle-gated loops) = **$2.09/day**, ~10× under the ~$20 target and ~25× under the $50 soft alert, *with* the round-up posture (retries charged, non-batch rates, no cache discount). Harness + evidence: `spikes/issue-001-cost-viability/` (results/af-001-evidence.2026-07-03.md). AF-001 flipped 🔴→🟢 in the feasibility register.
- **Notes / OD:** AF-040/041 (real-task cost acceptable; the $50/$100 defaults realistic) sit under the AF-001 umbrella; they are the threshold-realism half and are fast-follow, but the **viability target itself is blocking**. AF-042 (estimate-vs-invoice) and AF-043 (gate self-funding/accuracy) received initial evidence from this spike; their own reconciliation/shadow-window EVALs remain open.
